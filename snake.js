...
const config = {
  ...
  /**
	* @returns {string} Отдает текущий счет игры.
	*/
  getScoreCount() {
	 return document.querySelector(this.settings.scoreCount)
  },
...
const game = {
  ...
  reset() {
    ...
    this.snake.init(this.getStartSnakeBody(), 'up', this.config.getRowsCount() - 1, this.config.getColsCount() - 1);
	 // Инициализируем счетчик очков игры.
	 this.config.getScoreCount().innerHTML = 0;
  ...
  scoreCount() {
	return this.config.getScoreCount().innerHTML = this.snake.getBody().length - 1
  },
  /**
